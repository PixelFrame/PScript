...
$EtwLineRegex = '\[\d+\][\dA-F]{4}\.[\dA-F]{4}::(?<Time>\u200e\d{4}\u200e-\u200e\d{2}\u200e-\u200e\d{2} \d{2}:\d{2}:\d{2}\.\d{3}) \[Microsoft-Windows-LDAP-Client\](?<Message>.*)'
...
{
    Write-Progress -PercentComplete ($Progress++ / $LdapEtwContent.Count * 100) -Status "Line $Progress/$($LdapEtwContent.Count)" -Activity 'Processing file'
